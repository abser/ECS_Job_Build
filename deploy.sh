...
FAMILY="test-app"
echo $USER
Docker=`aws ecr get-login-password --region $REGION --profile $AWS_PROFILE`
...
NEW_TASK_DEFINITION=$(aws ecs register-task-definition --cli-input-json file://./task_def.json --region $REGION --profile $AWS_PROFILE) && \
...
SERVICES=`aws ecs describe-services --services ${SERVICE_NAME} --cluster ${CLUSTER_NAME} --region ${REGION} --profile ${AWS_PROFILE} | jq .failures[]`
echo "SERVICES: $SERVICES"
#Get latest revision
REVISION=`aws ecs describe-task-definition --task-definition ${NEW_TASK_ARN} --region ${REGION} --profile ${AWS_PROFILE} | jq .taskDefinition.revision`
echo "REVISION: $REVISION"
#Create or update service
if [ "$SERVICES" == "" ]; then
  echo "entered existing service"
  DESIRED_COUNT=`aws ecs describe-services --services ${SERVICE_NAME} --cluster ${CLUSTER_NAME} --region ${REGION} --profile ${AWS_PROFILE} | jq .services[].desiredCount`
  ...
  aws ecs create-service --cli-input-json file://./create_service.json  --region ${REGION} --profile ${AWS_PROFILE}
fi
